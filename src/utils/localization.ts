...
export const formatLocalizedDate = (date: string | Date, formatString: string, language: string) => {
...
export const formatLocalizedDistanceToNow = (date: string | Date, language: string, options?: { addSuffix?: boolean }) => {
  const dateObj = typeof date === 'string' ? new Date(date) : date;
  const locale = getDateFnsLocale(language);
